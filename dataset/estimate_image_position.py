...
with connect() as conn:
    ...
    for anchor in anchors.values():
        east, north = latlon_to_local_meters(anchor["lat"], anchor["lon"], origin_lat, origin_lon)
        anchor["east"] = east
        anchor["north"] = north
    ...
    search_index = build_or_load_search_index(conn, anchors)
    ranked = rank_candidates(QUERY_PATH, anchors, search_index)

print(f"Anchors in DB: {len(anchors)}")
print(f"Cheap search ranked anchors: {len(ranked)}")
print()

sift = cv2.SIFT_create(nfeatures=args.max_features) if args.max_features > 0 else cv2.SIFT_create()
...
for step in candidate_steps:
    if step == 0:
        selected = [anchor for _, anchor in ranked]
        step_name = "all"
    else:
        selected = [anchor for _, anchor in ranked[:step]]
        step_name = str(step)

    new_candidates = [anchor for anchor in selected if anchor["path"] not in checked_paths]
    if not new_candidates:
        continue

    print(f"SIFT verifying candidate step {step_name}: {len(new_candidates)} new candidates")

    with connect() as conn:
        cached_features = load_cached_anchor_features(conn, [anchor["path"] for anchor in new_candidates])
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            estimate = make_estimate(anchor, features, match_result, pixel_to_meter_matrix, origin_lat, origin_lon)
    ...
    print(f"  found usable matches this step: {len(estimated_items)}")
    print(f"  total usable matches: {len(all_matches)}")

    for item in estimated_items[:5]:
        print(
            f"    {item['anchor_path']} | "
    ...
    if len(all_matches) >= args.needed_matches:
        print(f"Stopping early: got {len(all_matches)} matches, needed {args.needed_matches}")
        break

if len(all_matches) == 0:
    raise RuntimeError("No usable matches found. Try --candidate-steps 300,700,0 or lower thresholds.")
